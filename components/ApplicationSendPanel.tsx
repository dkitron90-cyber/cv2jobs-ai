...
type ApplicationSendPanelProps = {
  ...
  file: File;
  onClose: () => void;
  onComplete: (channel: ApplyChannel, mode: SendMode, outcome: SendOutcome) => void;
...
export default function ApplicationSendPanel({
  ...
  const assistedReady =
    mode === "recruiter"
      ? outreachCopied || Boolean(recruiterChannel) || cvReady
      : letterCopied || Boolean(portalChannel) || cvReady;
  ...
  async function handleAutoSend() {
    ...
    if (result.ok) {
      ...
      onComplete("email", "recruiter", "verified");
      return;
    }
  ...
  function handleAssistedComplete() {
    if (!assistedReady) return;
    if (mode === "recruiter") {
      onComplete(recruiterChannel ?? (application.contactEmail ? "email" : "none"), "recruiter", "assisted");
      return;
    }
    onComplete(portalChannel ?? "portal", "portal", "assisted");
  }
    ...
    <div className="send-panel-backdrop" role="presentation" onClick={onClose}>
      ...
      >
          ...
          <>
            ...
            {autoAvailable && application.contactEmail ? (
              <section className="send-auto-block">
                ...
                {!user ? (
                  <div className="send-auto-signin">
                    <p className="send-panel-note">{t("matcher.sendAutoSignIn")}</p>
                    {onRequestSignIn && (
                      <button type="button" className="send-step-action primary" onClick={onRequestSignIn}>
                        {t("auth.signIn")}
                      </button>
                    )}
                  </div>
              ...
              <p className="send-assisted-banner">{t("matcher.sendAssistedBanner")}</p>
            )}

            <p className="send-manual-divider">{t("matcher.sendManualDivider")}</p>
            <ol className="send-panel-steps">
              <li className={cvReady ? "done" : ""}>
                <span className="send-step-mark">1</span>
                <div>
                  <strong>{t("matcher.sendStepCvPrepare")}</strong>
                  <button type="button" className="send-step-action" onClick={handleDownloadCv}>
        ...
        <footer className="send-panel-footer">
          <p className="send-panel-note">{t("matcher.sendAssistedExplain")}</p>
          <button
            type="button"
            className="send-panel-done"
            onClick={handleAssistedComplete}
            disabled={!assistedReady}
          >
            {mode === "recruiter" ? t("matcher.sendMarkAssistedOutreach") : t("matcher.sendMarkAssistedApply")}
          </button>
        </footer>
      </section>
    </div>
